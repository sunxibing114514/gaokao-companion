...
### 2. 句子语音搜索(全局快捷键 `Ctrl+Alt+S`)
- 弹出**上部居中**的搜索框,透明度可配置(**默认 30**)
- 句子来自外部 JSON(格式:`{ "句子": "声音链接", ... }`)
- 输入筛选 → ↑↓ 选择 → 回车/双击播放对应声音(**不循环**)
- `Ctrl+Alt+X` 关闭正在播放的声音

### 3. 网易云音乐搜索(全局快捷键 `Ctrl+Alt+M`)
...
- `Ctrl+Alt+X` 停止播放;`Esc` 关闭搜索框;失焦自动关闭
...
| `Ctrl+Alt+S` | 句子语音搜索框 |
| `Ctrl+Alt+M` | 网易云音乐搜索框 |
| `Ctrl+Alt+X` | 停止当前播放(语音/音乐) |

> 组合键需同时按住(按住 Shift 或 Win 时不触发,避免误触);由低级键盘钩子**只监听不拦截**,不会影响其它软件的快捷键。停止键选用 X 以避开截图/OCR 类工具常用的 Ctrl+Alt+C。

## 配置文件(config.json)

首次运行自动生成,**优先 exe 同目录;该目录不可写时(如带只读属性/被占用)自动回退到 `%APPDATA%\GaokaoCompanion\config.json`**。数据源 JSON 支持把倒计时对象和句子映射对象拼在同一个文件里(两个 URL 可以指向同一地址),程序会自动挑出各自需要的对象。

> **手工编辑友好**:程序只会回写组件位置两个字段(合并保存),你手工修改的其它内容(数据源地址、Cookie、壁纸规则等)不会被程序覆盖——运行中编辑也一样。设置窗口里的修改则需点击「保存并应用」落盘。示例见 [`config.example.json`](config.example.json):
